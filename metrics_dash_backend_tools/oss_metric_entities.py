...
class Repository(OSSEntity):
    ...
    def get_path_to_json_data(self,metrics_data_path):
        """
        Derive the path for json data using json parent
        path and extension
        ...
        Returns:
            String path to data.
        """
        return self.get_path_to_data(metrics_data_path, "json")

    def get_path_to_report_data(self,reports_data_path):
        """
        Derive the path for markdown data using markdown
        parent path and extension
        ...
        Returns:
            String path to data.
        """
        return self.get_path_to_data(reports_data_path, "md")

    def get_path_to_resource_data(self,graphs_data_path, resource_name, fmt="png"):
        """
        ...
        data_path = os.path.join(graphs_data_path, id_str)
        pathlib.Path(data_path).mkdir(parents=True, exist_ok=True)
...
class GithubOrg(OSSEntity):
    ...
    def get_path_to_resource_data(self,graphs_data_path, resource_name, fmt="png"):
        ...
        parent_path = os.path.join(graphs_data_path, f"{self.login}")
        pathlib.Path(parent_path).mkdir(parents=True, exist_ok=True)
